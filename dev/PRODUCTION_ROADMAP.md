...
| Multi-tenancy / RLS maturity | **90** | Complete + tested + dark; roles provisioned in dev; enabled on **0** services |
...
**Phase 2 — Enable RLS canary**  — *canary (customer-service) **DONE** 2026-07-16; rollout (3) not started*
- Commits: (1) **DONE** `d1af8ad` — enable `rls` profile for customer-service (`spring.profiles.include: rls` in its own `application.yml`; CI-verified); (2) **DONE** `7ba6391` — boot-level isolation IT, **JWT-driven** (`CustomerRlsJwtIsolationIntegrationTest`, CI: 3/3); (3) roll to los→lms→ledger→payment→bre→partner→notification (per-service commits) — **not started**.
...
- Services: per canary order. Tests: per-service RLS ITs (extend harness).
- Success: `current_user=originex_app`, `app.tenant_id` set per tx, cross-tenant read returns 0 rows on a booted service. *(Met for customer-service: `originex_app`×10 connections, row persisted under the JWT's tenant, cross-tenant GET → 404.)*

> **Two corrections this phase surfaced — read before doing (3).**
> 1. **A test is not an enablement.** `@ActiveProfiles("rls")` activates the profile for that test's context only. Commit (1) was mis-assessed as "already done" because rls-profile ITs existed and passed — while no service config or CI step enabled it anywhere. Check `services/<svc>/src/main/resources/` and `infra/helm/`, not the test tree.
> 2. **Header-driven RLS tests prove nothing about the authenticated path.** `TenantResolutionFilter` (`X-Tenant-Id`) is registered only while `originex.security.enabled=false`; with security on it does not exist and `TenantClaimResolutionFilter` reads the verified `tenant_id` claim. Each service therefore needs a JWT-driven IT (template: `CustomerRlsJwtIsolationIntegrationTest`), not just a header one. Full rationale: `dev/RLS_ENABLEMENT.md`.

> **Sequencing risk for (3).** `lms` is second in the canary order but its `LoanLifecycleIntegrationTest.disbursementThroughRepaymentReachability` has failed every CI run since 2026-07-16 11:00 (issue #3) — unrelated to RLS. Resolve or resequence before enabling lms; a service whose ITs are already red cannot evidence its own canary.

> **Deployment is still dark.** No `infra/helm` values set `SPRING_PROFILES_ACTIVE`, so RLS is enabled in config and tests but in **no deployed environment**. Phase 2 does not close until that is deliberate.
