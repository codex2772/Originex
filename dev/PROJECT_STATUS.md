...
| **partner-integration** (8087) | Bureau/Aadhaar/PAN/bank verification | 45% | 4 (`/v1/partner`: credit-bureau, aadhaar, pan, bank-account) | ❌ none | — | — | V1–V2 (integration_requests) | hardened, inert | `@PreAuthorize` split: `partner:credit-pull` (los) / `partner:verify` (customer); **armed but dormant** — must NOT flip to ENFORCED until los+customer send tokens (**T4**; partner is hit by TWO sync callers). | live vendors (sandbox stubs), RLS enable, **T4 S2S token before enforce** |
