...
1. **Ledger postings fail today.** Any `LoanDisbursed`, `RepaymentAllocated`, or `InterestAccrued` event that reaches `ledger-service` will throw `IllegalArgumentException: Account not found` for the pool/interest legs, because `POOL_ACCOUNT_ID`, `INTEREST_INCOME_ID`, and `INTEREST_RECEIVABLE_ID` are hardcoded UUIDs with no corresponding seed rows in `account_snapshots`. If you're testing the disbursement-to-ledger flow end to end, seed these three accounts first or expect a stack trace, not silent failure.
2. **`ledger-service` and `payment-service` are missing `inbox_events`/`outbox_events` tables** in their Flyway migrations, despite both services actively using `InboxEventRepository`/`OutboxPublisher`. If `spring.jpa.hibernate.ddl-auto` is ever set to `validate` (a reasonable thing to do before a "production-grade" claim), these two services will fail to start. Payment-service is worse off — it has *neither* table, contradicted by a migration comment falsely claiming they're "auto-created by the starter" (`OutboxAutoConfiguration` only registers beans/repositories, it issues no DDL).
3. **Fresh local dev is broken for 3 of the 8 "complete" services.** `dev/init-scripts/init-databases.sql` creates `originex_los, originex_lms, originex_ledger, originex_payment, originex_customer, originex_collections, originex_iam, originex_template` — it does **not** create `originex_bre`, `originex_partner`, or `originex_notification`, even though those three services' `application.yml` point at exactly those DB names. `docker compose up -d` followed by starting bre-service/partner-integration-service/notification-service will fail on missing database. Meanwhile it creates DBs for `collections-service` and `iam-service`, which don't exist as Maven modules at all.
4. **Payment rail auto-selection never picks NEFT.** `selectRail()`'s threshold ordering makes the NEFT branch unreachable dead code — every amount either satisfies `>=₹2L→RTGS` or falls into `<₹2L` which always also satisfies `<=₹5L→IMPS`. If NEFT is meant to be the default/cheap rail for mid-size payments, this is a live logic bug, not a documentation gap.
...
| 22 | Data architecture table: 8 databases (`originex_customer`, `..._los`, `..._lms`, `..._ledger`, `..._partner`, `..._payment`, `..._notification`, `..._bre`) | `dev/init-scripts/init-databases.sql` actually creates `..._los, ..._lms, ..._ledger, ..._payment, ..._customer, ..._collections, ..._iam, ..._template` — **missing** `..._bre`, `..._partner`, `..._notification`; **extra** DBs for unbuilt/scaffold services | **High-impact** — breaks fresh local dev for 3 real services |
...
4. **Before running fresh local dev**, patch `dev/init-scripts/init-databases.sql` to create `originex_bre`, `originex_partner`, `originex_notification` (or drop the unused `originex_collections`/`originex_iam`/`originex_template` entries if those aren't needed yet).
5. **If the task touches payment rail selection**, decide whether NEFT's unreachability is intentional (unlikely) before changing thresholds — this is a one-line fix (add an upper bound to the RTGS check) but worth flagging to the user rather than silently "fixing" business logic.
