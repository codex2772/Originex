...
| `template-service` | **8089** (was 8080 — moved in Phase 0 commit 5, "Normalize local development ports") | Scaffold / reference implementation | Handover claimed "port not assigned" — **false**, `application.yml` explicitly set `server.port: 8080`, which **collided with Kafka UI**. **RESOLVED**: moved to 8089, the project's own documented "next available port" convention. `HexagonalArchitectureTest` exists with 4 ArchUnit rules (domain-no-application, application-no-adapter, domain-no-spring, inbound-adapter-no-outbound-adapter) — note there is no separate "no JPA" rule, only "no Spring," though JPA usage would transitively be caught. |
...
- **`OutboxPoller`**: polls every 500ms (`originex.outbox.poll-interval-ms`, default 500), batch size 100, plus a daily 2am cleanup job removing published events older than 7 days. Its `resolveTopicFromEventType()` originally had **only 5 domain-prefix mappings**: `originex.los.*`, `originex.lms.*`, `originex.customer.*`, `originex.ledger.*`, `originex.payments.*`, with no mapping for `originex.bre.*`, `originex.notification.*`, or `originex.partner.*` (any such event would have fallen through to `originex.platform.unrouted-events`). **RESOLVED — Phase 0 commit 6** ("Normalize OutboxPoller routing / Kafka topics"): all 3 missing prefixes now route to reserved topics provisioned in the same commit, even though none of those 3 services publish via outbox yet — this is preventative, matching the file's own existing precedent of pre-provisioning the unbuilt `collections` domain.
...
2. ~~**`ledger-service` and `payment-service` are missing `inbox_events`/`outbox_events` tables**~~ **RESOLVED — Phase 0 commits 1, 2, and 7.** `ledger-service` got `inbox_events` in commit 1; `payment-service` got both `outbox_events` and `inbox_events` in commit 2; and the narrower related gap — `ledger-service`'s `outbox_events` missing the `published_at` column — was closed in commit 7 (`V3__add_outbox_published_at.sql`).
...
6. ~~**`OutboxPoller.resolveTopicFromEventType()` has no route for BRE, notification, or partner-integration event prefixes.**~~ **RESOLVED — Phase 0 commit 6.** All 3 now route to reserved, clearly-documented-as-unused topics.
7. ~~**`originex.customer.customers.events`** — no Strimzi `KafkaTopic` CRD~~ **RESOLVED — Phase 0 commit 6.** CRD added; this was the one live, non-speculative gap in this list (customer-service actively publishes here today).
...
12. ~~**template-service's port (8080) collides with Kafka UI's port (8080)**~~ **RESOLVED — Phase 0 commit 5.** `template-service` moved to 8089. A second, previously-undocumented conflict was found in the same investigation and fixed in the same commit: `dev/docker-compose.yml`'s `schema-registry` container also bound host port 8081, colliding with `customer-service`'s `server.port: 8081` — `schema-registry`'s host-side mapping moved to 8090 (container still listens on 8081 internally; no Docker-internal URL changed). A full repo-wide port audit (`dev/PHASE0_VERIFICATION.md`) confirmed no other host-port conflicts exist among the 9 services + 5 infra containers.
...
| 19 | `template-service` port "not assigned" | **RESOLVED — Phase 0 commit 5.** `server.port` was explicitly set to `8080`, colliding with Kafka UI's documented port 8080; moved to `8089`. | Was a real, reproducible port conflict, mis-described as absent; now fixed |
| 20 | Kafka topics: "30+ topics defined" | `infra/kafka/topics.yaml` defined exactly **12** `KafkaTopic` CRDs at audit time; now **16** after Phase 0 commit 6 added the missing `customer.customers.events` (real gap) plus 3 reserved/pre-provisioned topics for bre/partner/notification (documented as unused, not backing any live publisher) | Was overstated by ~2.5x; still overstated (16 vs. "30+"), historical finding stands |
| 21 | (implicit) `originex.customer.customers.events` topic exists | **RESOLVED — Phase 0 commit 6.** CRD added — this was the one live, non-speculative gap (customer-service actively publishes here today) | Was infra/code drift; now fixed |
...
2. ~~**Before adding a Kafka event**, check `OutboxPoller.resolveTopicFromEventType()` (§3) for routing and `infra/kafka/topics.yaml` for a matching CRD — both are currently incomplete for `bre`/`notification`/`partner` domains.~~ **Done as of Phase 0 commit 6** — routing and reserved topics now exist for all 3; still true for any *new* domain not yet covered.
...
|---|---|---|---|
| **RESOLVED — commit 7** (`3-`… see log) | Align ledger `outbox_events` schema with shared `OutboxEventJpaEntity` | `ledger-service`'s `outbox_events` table (from V1) was missing the `published_at` column that `OutboxEventJpaEntity` maps and that `customer-service`/`los-service`/`lms-service`'s `outbox_events` tables all have. Turned out to be worse than a boot-time `validate` failure: `published_at` is written by `OutboxPoller.markPublished()` on every publish and read by `deletePublishedBefore()`, so ledger's outbox would also fail at runtime on its first published event. Fixed by `V3__add_outbox_published_at.sql` (additive `ALTER TABLE`; V1 applies cleanly so it was left untouched). | Found commit 2 (`dev/PHASE0_VERIFICATION.md`, "AFTER commit 2"); fixed commit 7 |
| **RESOLVED — `fix(lms)` Kafka serde commit** | Add LMS Kafka `byte[]` serdes | `lms-service`'s `application.yml` was **missing all four Kafka serde settings** (`key/value-deserializer`, `key/value-serializer`) that `payment-service` and `notification-service` already declare. LMS's inbound adapters consume `ConsumerRecord<String, byte[]>` and the outbox publishes via `KafkaTemplate<String, byte[]>`, so with the container defaults LMS could **neither consume `DisbursementRequested` nor publish `LoanDisbursed`/`InterestAccrued`/`RepaymentAllocated` at runtime** — the loan lifecycle would have stalled regardless of the `createLoan` disbursement wiring. Fixed by adding the four serdes (matching the two working services); additive config change only. | Found while building the E2E integration-test harness (`LoanLifecycleIntegrationTest`), which now validates LMS consume+publish end-to-end |
| **RESOLVED — DPD/settlement prerequisite commit** | Rehydrate `Loan` child collections on load | `LoanJpaEntity.toDomain()` returned **empty** `installments`/`disbursements`/`charges` on every load, and `save()` detached-merges with `orphanRemoval=true` — so child rows were orphan-deleted after the first post-creation save, and no loaded loan ever saw its schedule or disbursements. This silently **broke `confirmDisbursementByPayment`** (empty disbursements → the INITIATED disbursement was never found → the loan never reached ACTIVE via the payment path) and the schedule GET endpoint, and blocked any installment-level repayment logic. Fixed by rehydrating children when initialized (`Hibernate.isInitialized` guard keeps the non-transactional accrual path safe), force-initializing them in `findById`, and re-loading with children in the accrual/DPD processors so their saves don't wipe the schedule. `save()`/optimistic-locking mechanism unchanged. | Found while implementing installment settlement (repayment needs the schedule in the domain; the loan must reach ACTIVE first) |
| Open (deferred) | NPA interest-suspense, provisioning, and auto-upgrade | DPD/NPA classification now runs (`DpdAgingService`), but v1 **persists classification only**: no interest-suspense reclassification on NPA (see `Loan.accrueInterest` Javadoc), no provisioning ledger postings, no `AssetClassificationChanged` event, and no automatic NPA→ACTIVE upgrade on cure (DPD resets to 0 but status stays NPA). Each is a separate, coordinated increment. | Deferred by design during Interest Accrual and DPD/NPA v1 |
| Open | Normalize the `Loan` interest-rate unit | `Loan.interestRate` is stored as an annual **percentage** (e.g. `12.5`), while `ScheduleGenerator`'s parameter is documented as a **fraction** (`0.12`) — reconciled ad-hoc by `LoanApplicationServiceImpl.createLoan` via `movePointLeft(2)`. `InterestAccrualCalculator` mirrors the same conversion (÷100 then ÷365) to stay consistent — a fraction basis would have accrued 100×. Normalizing the unit on the aggregate is a **separate, coordinated** change: it touches EMI calculation, interest accrual, and future repayment/penalty logic, so it must not be done piecemeal. | Found while implementing Interest Accrual (see `InterestAccrualCalculator` TODO) |
