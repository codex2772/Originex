...
7a. **Resilience4j retries + masks 4xx as "temporarily unavailable" (observability smell):** Partner-adapter methods (e.g. `PartnerIntegrationPanAdapter.verify`) are wrapped in `@Retry` + `@CircuitBreaker` with no `ignoreExceptions`, so an HTTP **4xx client error** (a validation/contract bug, which retrying can never fix) is retried and then swallowed by the fallback, which returns a generic *"service temporarily unavailable. Please retry."* The caller then throws, surfacing a **validation bug dressed as an outage** — misdirecting whoever debugs it next. Fix pattern: configure `ignoreExceptions` for `HttpClientErrorException` (4xx) so client errors fail fast and surface their real cause, and reserve retry/fallback for 5xx/transport faults. The PAN `referenceId` fix (§ customer registration) removes the *trigger* for the PAN adapter specifically, but the retry-and-mask pattern lives on in every other partner adapter (Aadhaar, bank-account, bureau).
